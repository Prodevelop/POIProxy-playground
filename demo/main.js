    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            var loadNews = function(channel) {
                ...
                if (channel) {
                    $.ajax({
                        url: config.url + '/browse?z=0&y=0&x=0&service=' + channel,
                        jsonp: "callback",
                        dataType: "jsonp",
                        error: function() {},
            ...
            var loadService = function(service) {
                ...
                if (service) {
                    $.ajax({
                        url: config.url + '/browseByLonLat?&service=' + service + "&lon=" + lon + '&lat=' + lat + '&dist=1000',
                        jsonp: "callback",
                        dataType: "jsonp",
                        error: function() {},
                        success: function(response) {
        ...
        error: function(err) {
            alert("An error happened receiving the config.json");
        }
    });
});